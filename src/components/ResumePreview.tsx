...
type StyleCtxType = AccentColors & {
  sectionHeaderCase: "normal" | "uppercase";
};
const StyleCtx = createContext<StyleCtxType>({
  ...ACCENT.black,
  sectionHeaderCase: "uppercase",
});

/* ── Per-element style context ── */
const ElementStyleCtx = createContext<{
  styles: Record<string, ElementStyle>;
  setStyle: (key: string, style: ElementStyle) => void;
} | null>(null);
...
function elementInlineStyle(s: ElementStyle): React.CSSProperties {
  return {
    ...
    textTransform:
      (s.textTransform as React.CSSProperties["textTransform"]) || undefined,
...
export default function ResumePreview({
  ...
  const sectionTitles = useMemo(
    () => ({ ...DEFAULT_TITLES, ...resume.sectionTitles }),
  ...
  function setElementStyle(key: string, style: ElementStyle) {
    upd({ styleOverrides: { ...elementStyles, [key]: style } });
  }
  ...
  function handleProjDragEnd(event: DragEndEvent) {
    ...
    if (!over || active.id === over.id) return;
    upd({
      projects: arrayMove(resume.projects, Number(active.id), Number(over.id)),
    });
  }
  ...
  function handleBulletDragEnd(expIdx: number, event: DragEndEvent) {
    ...
    upd({
      experience: resume.experience.map((e, i) =>
        i === expIdx ? { ...e, bullets: newBullets } : e,
      ),
    });
  }
      ...
      case "summary":
        return (
          <EditableSection
            ...
            title={title}
            onTitleChange={
              onUpdate ? (v) => setSectionTitle(key, v) : undefined
            }
            onDelete={
              onUpdate
                ? () =>
                    upd({ sectionOrder: sectionOrder.filter((s) => s !== key) })
                : undefined
            }
            editable={!!onUpdate}
          >
            <EBlock
              ...
              onSave={onUpdate ? (v) => upd({ summary: v }) : undefined}
              styleKey="summary"
            />
          </EditableSection>
        );
        ...
        return (
          ...
          >
            {onUpdate ? (
              ...
              >
                <SortableContext
                  items={resume.experience.map((_, i) => i)}
        ...
        return (
          ...
          >
            {onUpdate ? (
              ...
              >
                <SortableContext
                  items={resume.projects.map((_, i) => i)}
                  strategy={verticalListSortingStrategy}
                >
        ...
        return (
          ...
          >
            {onUpdate ? (
              ...
              >
                <SortableContext
                  items={resume.education.map((_, i) => i)}
                  strategy={verticalListSortingStrategy}
                >
              ...
              <button
                onClick={() =>
                  upd({
                    customSections: {
                      ...resume.customSections,
                      [key]: { title: title, items: [...items, "New item"] },
                    },
                  })
                }
                className="text-sm text-blue-500 hover:text-blue-700 flex items-center gap-1.5 mt-1.5 py-1"
...
function EditableSection({
  ...
  return (
    <div className="mb-4">
      <div
        className="flex items-center gap-1 border-b pb-0.5 mb-2"
        ...
        <div className="absolute top-full mt-1 left-1/2 -translate-x-1/2 bg-white border border-gray-200 rounded-lg shadow-lg py-1 z-10 min-w-[180px] text-left">
          ...
          <div className="border-t border-gray-100 mt-1 pt-1 px-2 pb-1">
            ...
            >
              ...
              <button
                type="submit"
                className="text-xs text-white bg-blue-500 hover:bg-blue-600 rounded px-2 py-1"
              >
                Add
              </button>
            </form>
...
function SortableExpItem({
  ...
  onBulletDragEnd: (expIdx: number, event: DragEndEvent) => void;
}) {
  const {
    attributes,
    listeners,
  ...
  return (
    <div ref={setNodeRef} style={style} className="mb-4 relative group/exp">
      ...
      <DndContext
        sensors={sensors}
        collisionDetection={closestCenter}
        onDragEnd={(e) => onBulletDragEnd(idx, e)}
      >
        <SortableContext
          items={exp.bullets.map((_, j) => j)}
          strategy={verticalListSortingStrategy}
        >
            ...
            {exp.bullets.map((b, j) => (
              <SortableBullet
                ...
                onSave={(v) => {
                  ...
                  upd({
                    experience: resume.experience.map((e, k) =>
                      k === idx ? { ...e, bullets } : e,
                    ),
                  });
                }}
...
function SortableBullet({
  ...
  return (
    <li
      ref={setNodeRef}
      style={style}
      className="text-gray-700 group/bullet flex items-start gap-0.5"
    >
      <span
        {...attributes}
        {...listeners}
...
function SortableEduItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex flex-wrap justify-between items-baseline mb-1 gap-x-1 relative group/edu"
    >
...
function E({
  ...
  const cls = [
    bold || elemStyle.fontWeight === "bold" ? "font-bold text-gray-900" : "",
    small ? "text-xs" : "",
    muted
      ? "text-gray-500"
      : !bold && elemStyle.fontWeight !== "bold"
        ? "text-gray-700"
        : "",
  ]
    .filter(Boolean)
        ...
        <span className="inline-flex items-center gap-1">
          <button
            ...
            onMouseDown={(e) => {
              e.preventDefault();
              if (inputRef.current)
                applyFormat(inputRef.current, draft, setDraft);
            }}
          ...
          <input
            ...
            style={{
              width: wide
                ? "100%"
                : `${Math.max((draft.length || 8) + 2, 8)}ch`,
              minWidth: "6ch",
              ...inlineStyle,
            }}
          />
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-1.5 mb-0.5">
          <button
            ...
            onMouseDown={(e) => {
              e.preventDefault();
              if (taRef.current) applyFormat(taRef.current, draft, setDraft);
            }}
            title="Bold selected text (Ctrl+B)"
